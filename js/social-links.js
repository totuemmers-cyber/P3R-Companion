...
function slRenderRiskBoard() {
  ...
  const renderGroup = (title, entries) => {
    ...
    return `<div class="sl-risk-group"><h3>${title}</h3>${entries
      .map((item) => {
        ...
        if (item.isRare) {
          reasons.push(item.actionableToday ? 'rare but available now' : 'rare later this week');
        }
        if (item.setupNeeded) {
          reasons.push('needs setup');
        }
  ...
  container.innerHTML = renderGroup('Can still act', actionable) + renderGroup('Watch later / blocked', blocked);
}
...
function slRenderPremiumWeek() {
  ...
  for (let index = 0; index < 7; index += 1) {
    ...
    ['day', 'evening'].forEach((timeSlot) => {
      slGetCalLinks(date, null, timeSlot)
        .filter((item) => item.weeklyPressure > 0 && (item.isRare || item.deadlineInfo.isSoon))
        .slice(0, 2)
        .forEach((item) => {
          premiumItems.push({
            ...item,
  ...
  container.innerHTML = premiumItems
    ...
    .map((item) => {
      ...
      if (isCurrentBest) {
        tags.push(item.timeSlot === 'day' ? 'Best today' : 'Best tonight');
      } else {
        tags.push('Watch later this week');
      }
      if (item.isRare) {
...
function slRenderRecommendations() {
  ...
  let html = `<section class="sl-rec-primary"><div class="sl-rec-header"><span class="sl-rec-kicker">Best today</span><span class="sl-rec-secondary">Today priority ${Math.max(
    0,
    Math.round(primary.todayPriority)
  ...
  if (backups.length) {
    html += `<section class="sl-rec-group"><h3>Good backup today</h3>${backups
      .map((item) => {
  ...
  if (blockedImportant) {
    html += `<section class="sl-rec-group"><h3>Watch later this week</h3><div class="sl-rec-item blocked-callout"><div class="sl-rec-info"><div class="sl-rec-name-row"><span class="sl-rec-name">${blockedImportant.link.character}</span><span class="sl-rec-arcana">${blockedImportant.arcana}</span></div>${renderTags(
...
function slRenderMyLinks() {
  ...
  const models = slGetModelsForCurrentState()
    ...
    .sort((left, right) => {
      if (sortMode === 'today') {
        return Number(right.actionableToday) - Number(left.actionableToday) || right.todayPriority - left.todayPriority;
      }
...
function slGetCalLinks(date, dayOfWeek, timeSlot) {
  ...
  const filtered = results.filter((item) => {
    if (mode === 'urgent') {
      return item.weeklyPressure > 0 && (item.deadlineInfo.isSoon || item.isRare || !item.actionableToday);
    }
    if (mode === 'actionable') {
      return item.actionableToday;
    }
    return true;
  });
...
function slCalLinkHtml(item, timeSlot, options = {}) {
  ...
  let flagText = '';
  if (options.recommended) {
    flagText = options.recommendedLabel || 'Best today';
...
function slRenderCalendar() {
  ...
  for (let index = 0; index < 7; index += 1) {
    ...
    const premiumDay = [...dayLinks, ...eveningLinks].some((item) => item.weeklyPressure > 0 && (item.isRare || item.deadlineInfo.isSoon));
    ...
    if (isToday && currentDayBest) {
      todaySummary.push(`Best day: ${currentDayBest.link.character}`);
    }
    if (isToday && currentEveningBest) {
      todaySummary.push(`Best eve: ${currentEveningBest.link.character}`);
    }
    ...
    html += `<div class="${cssClass}"><div class="sl-cal-day-header"><span class="sl-cal-day-name">${DAY_NAMES[dayOfWeek]}</span><span class="sl-cal-day-date">${slFormatDate(
      ...
      premiumDay ? '<span class="sl-cal-day-premium">Watch later this week</span>' : ''
